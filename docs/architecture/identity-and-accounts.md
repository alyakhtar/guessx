...
authenticated-versus-guest game, both participants may see a head-to-head
record scoped to the authenticated user's internal ID and the guest's current
...
using the same display name, is never shown for guest-versus-guest games, and
must not be used for account history, leaderboard calculations, authorization,
or any account claim. Authenticated-versus-authenticated records remain
stable-ID head-to-head statistics.
