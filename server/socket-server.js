...
function buildGameResultPayload(room) {
  const players = room.players;
  const player1 = players[0].name;
  const player2 = players[1].name;
  const winner = room.winner;
  const player1Identity = identityForGameResult(players[0]);
  const player2Identity = identityForGameResult(players[1]);
  ...
  return {
    ...
    player1IdentityKind: player1Identity.kind,
    player2IdentityKind: player2Identity.kind,
    ...(player1Identity.userId ? { player1UserId: player1Identity.userId } : {}),
    ...(player2Identity.userId ? { player2UserId: player2Identity.userId } : {}),
    ...(winnerIdentity?.userId ? { winnerUserId: winnerIdentity.userId } : {}),
    identityVersion: 1,
    gameDuration,
    totalGuesses,
